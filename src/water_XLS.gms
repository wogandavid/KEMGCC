...
parameter WAfconsumpmax(WAf,time,r,c);
parameter WAdemval(time,r,c) water demand in billions m3;
parameter WAexist(WAp,v,r,c) existing capacity of desal plants MMm3 per day (single) and GW (cogen);
Parameter WAcapital(WAp)    Current capital needed for new plants in MMUSD per GW or MMm3perday;
parameter WAomcst(WAp,r,c) Variable O&M cost for desal capacity USD per Mm3;
parameter WAELfixedOMcst(WAp,c) Fixed O&M costs USD per KW;
parameter WAELomcst(WAp,r,c) Variable O&M cost for cogen power plant USD per MWH;
$onecho > taskWA.txt
  par=WAfconsumpmax rng=WAfconsumpmax! Rdim=4
  par=WAdemval rng=WAdemval! Rdim=3
  par=WAexist rng=WAexist! Rdim=4
  par=WAcapital rng=WAcapital! Rdim=1
  par=WAomcst rng=WAomcst! Rdim=3
  par=WAELfixedOMcst rng=WAELfixedOMcst! Rdim=2
  par=WAELomcst rng=WAELomcst! Rdim=3
$offecho

$call GDXXRW C:\GitHub\KEM-GCC\data\WAdata.xlsx @taskWA.txt
execute_load "WAdata.gdx",
  WAfconsumpmax
  WAdemval
  WAexist
  WAcapital
